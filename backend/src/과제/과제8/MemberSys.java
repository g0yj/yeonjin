	...
	public static void main(String[] args) {
		...
		//3. 선택의 경우의 수 [ ch==1, ch==2]
			if(ch==1) {//회원가입
				...
				//* 객체 선언: 클래스명 객체변수명= new 생성자명(); <- Member에서 객체 만들어진거 아니라고 말했었음
				Member member = new Member();//null 출력. 설계시 초기값 없으면 기본값이 들어감. String 기본값은 null
				//* 객체 내 필드 수정
				// . 도트 연산자: 접근 연산자(스택변수가 알고 있는 객체주소로 감)
				member.id= inputId; member.password=inputPw; member.name=inputName;
				member.phone=inputPhone; member.age=inputAge;
